  ...
  function deletechecked(){
		...
		<script type="text/javascript">
			...
			if( /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent) ) {
 				window.location.href = "http://claytonfelt.net/mypage1/index.php";
		}
		*/
    ...
    if($sql_user == ""){
        //header("location:http://claytonfelt.net/mypage1/config/new_config.php");
        exit;
    } 
    ...
    if(!$db){
        ...
        header("location:config/new_config.php");
        exit;
    }
    ...
    $sql = "SELECT * FROM blocks where  page = '$page' AND type ='mainbox' ";
    $stmt = $db->query($sql);
	  $rows = $stmt->fetchAll();
	  $row = $rows[0];
    $mainbox_background = $row['background'];
    $xpos = $row['xpos'];
    $ypos = $row['ypos'];
    $width = $row['width'];
    $height = $row['height'];
    ...
    if ((strpos($current_browser,"Opera")=="true") OR ($current_browser=="Firefox")){
        ...
        $width = $width - (2 * substr($row['border'],0,1));
...
<table width="1024"  border="0" align="center" cellpadding="0" cellspacing="0" bordercolor="#000000" bgcolor"#000000">
    ...
    ?> 
    <!-- mainbox ----->
    <div width="80%"  style="position: relative; left: 0px; top: 0px; background-color: <?=$background;?>; height: <?=$row['height']+300;?>;">
    <?php
    if ($_SESSION['loggedin'] == "true"){
                ...
                <div id="<?=$block; ?>" 
                     style="border:<?=$border;?>;position: absolute;  top: <?=$ypos;?>px; left: <?=$xpos;?>px; width: <?=$width;?>px; height: <?=$height;?>px; background-color: <?=$mainbox_background;?>;" >
                     <div style="position: absolute; left: 0; top: 30; cursor:move;" onmousedown="dragStart(event, '<?=$row['id'];?>', '<?=$block;?>')">
                        <img src="images/4arrows.gif">
                    </div>
                  <?php }
    else {?>
        <div   id="<?=$block;?>" style="position: absolute; left: <?=$xpos;?>px; top: <?=$ypos; ?>px; width: <?=$width;?>px; height: <?=$height;?>px; background-color: <?=$mainbox_background;?>; border: <?=$row['border'];?>;">
        <?php } ?>
    <?php if ($_SESSION['loggedin'] == "true"){?>
        <a class="newpost" style="position: absolute; bottom: -20; right: 50;" href="<?=$_SERVER['PHP_SELF'];?>?action=edit_block&id=<?=$row['id'];?>&edit_params=yes&block_id=<?=$block;?>">
            Edit Block Parameters
        </a>
        <div onmousedown="document.move_page.border.value=<?=substr($row['border'],0,1);?>; resizeStart(event, '<?=$row['id'];?>', '<?=$block;?>','<?=$row['width'];?>','<?=$row['height'];?>')" style="position: absolute; bottom: -8; right: -9; cursor: nw-resize;" ><img src="images/right_corner.gif"></div>
         <?php }                                         
...
//---------------------------------- Type Top ------------------------------------        
        if ($row['type'] == "top"){
            ...
            if ($_SESSION['loggedin'] == "true"){?>
                <div id="<?=$block;?>" style='width:<?=$width1;?>;height:<?=$height1;?>;position: absolute; left: <?=$xpos;?>px; top: <?=$ypos;?>px;  border: <?=$row['border'];?> background-color: <?=$row['background'];?>; '>
                ...
                ?>
                <div id="<?=$block;?>" style='width:<?=$width1;?>;height:<?=$height1;?>;position: absolute; left: <?=$xpos;?>; top: <?=$ypos;?>;  border: <?=$row['border'];?>; background-color: <?=$row['background'];?>;'>
        ...
        if ($row['type'] == "general"){
            ...
            if ($_SESSION['loggedin'] == "true"){
                ?><div  id="<?=$block;?>"  style='padding:5px;position: absolute; left: <?=$xpos;?>px; top: <?=$ypos;?>px; height: <?=$height;?>px; width: <?=$width;?>px; background-color: <?=$background;?>; border: <?=$row['border'];?>;'>
                    ...
                    <span class="edit_box"  style="<?php if($width<350){?>font-size:14px;<?php } ?>left: 2px; bottom: -10px; cursor:pointer;">
                    	<a href="index.php?action=edit_block&id=<?=$row['id'];?>&block_id=<?=$block;?>&page=<?=$page;?>" style="text-decoration: none;">[EDIT CONTENT]</a>
                    </span>
                     <!-- Edit Block Parameters button -->
                ...
                <div   id="<?=$block;?>" class="rounded" style='padding:5px;position: absolute; left: <?=$xpos;?>px; top: <?=$ypos;?>px; height: <?=$height; ?>px; width: <?=$width;?>px; background-color: <?=$background;?>; border: <?=$row['border'];?>' ><?php 
                	} ?><?=$row['content'];?></div>
        <?php }  
        //------------------------------- Type include ---------------------
         if ($row['type'] == "include"){
            ...
            if ($_SESSION['loggedin'] == "true"){
                ...
                <div   id="<?=$block;?>" class="rounded" style='padding:5px;position: absolute; left: <?=$xpos;?>px; top: <?=$ypos;?>px; height: <?=$height; ?>px; width: <?=$width;?>px; background-color: <?=$background;?>; border: <?=$row['border'];?>' >
                	<?php 
                	}
                	 //include($row['content']); 
                	 ?>
                	 <iframe src="<?=$row['content'];?>" style="width: 100%; height: 100%;">?></iframe>
                	 </div>
        <?php }
//------------------------------- Type youtube ---------------------------------
        if ($row['type'] == "youtube"){
...
//------------------------------- Type blog ---------------------------------
        if (stristr ($row['type'],"blog")){
          if($row['style_type']!="index"){
            ...
            if ($_SESSION['loggedin'] == "true"){
                ...
                <div id="<?=$block;?>"  style='overflow: auto;  position: absolute; left: <?=$xpos;?>px; top: <?=$ypos;?>px;  <?php if($height != 0){echo "height:". $height."px;";} ?> width: <?=$width;?>px; background-color: <?=$background;?>; border: <?=$row['border'];?>;'>
                   
                    <!-- New post button -->
                    <a class="newpost" style="position: absolute; top: 5px;" href="blog_edit.php?type=<?=$row['type'];?>">
                        New Post
                    </a>
                   
                    <!-- Edit Block Parameters button -->
                    <a class="edit_params" style="position: absolute; top: 5px; left: 215px;" href="<?=$_SERVER['PHP_SELF'];?>?action=edit_block&id=<?=$row['id'];?>&edit_params=yes&block_id=<?=$block;?>&page=<?=$page;?>">
                        Edit Block Parameters
                    </a>
                    <!-- Move block arrows -->
                    <div style="position: absolute; top: 5px; left: 510px;font-size: .9em;width: 180; cursor:move; "
                ...
                <div   id="<?=$block;?>" style='overflow: auto;   position: absolute; left: <?=$xpos;?>px; top:<?=$ypos;?>px; <?php if($height != 0){echo "height:". $height."px;";} ?> width: <?=$width;?>px; ?>; background-color: <?=$background;?>; border: <?=$row['border'];?>' >
                    <?php } ?>
                    <div style="padding-left: 1em; padding-right: 1em; width: <?=$width;?>;"><?php
                        
                        //edit_block_params($id, $block);
                        $type = $row['type'];
                        
                        if ($selected_year == ''){
                        	$sql = "SELECT * from $type order by date DESC limit 1";
                        	}
                       else {
                        ...
                        //-- Blog loop --    
                              ?>
                              <a href="make_recipe_pdf.php?type=<?=$type;?>&id=<?=$row_blog['id'];?>">Print</a>
                              <br>
                              <!-- Blog title -->
                              ...
                              if ($_SESSION['loggedin'] == "true"){?>
                                  <a  title="Edit Post" class="edit_params" style="position: absolute; top: 5px; left: 110px;" href="blog_edit.php?type=<?=$row['type'];?>&blogid=<?=$row_blog['id']; ?>&page=<?=$row['page'];?>">
                                      Edit Post
                                  </a>
                              <?php } ?>
        ...
        //-------------- blog index block ------- 
            ...
            if ($_SESSION['loggedin'] == "true"){
                ?><div  id="<?=$block;?>"  style='padding:5px;position: absolute; left: <?=$xpos;?>px; top: <?=$ypos;?>px; height: <?=$height;?>px; width: <?=$width;?>px; background: <?=$background;?>; border: <?=$row['border'];?>;'>
                    <div style="position: absolute; left: 0; top: 30; cursor:move;" onmousedown="dragStart(event, '<?=$row['id'];?>', '<?=$block;?>')">
                        <img src="images/4arrows.gif">
                    </div>
                ...
                <div>
                   ...
                   for($x=0; $x<$total_links; $x++) {
                        ...
                        <div style="margin:10px;">
                            ...
                            <a  href="index.php?action=edit_download&id=<?=$x;?>&page=<?=$page;?>&block_id=<?=$row['id'];?>">[EDIT]</a><br>
                        </div>    
                        <?php }
                        else { ?>
                        <div style="margin:10px;">
               ...
               <?php if ($action != "upload_video" && $action != "convert_video" && $action != "ftp_video" && $action != "edit_video_data"){?>
                    ...
                    <div style="position: absolute; right: 10; top: 40; border: 4px solid white; width: 480px;display:none;">
                      ...
                      <script type="text/javascript" defer="defer">
                      		...
                      		fo.addVariable("config", "{  playList: [ { overlayId: 'play' },{ url: 'http://claytonfelt.net/mypage1/videos/<?=$video;?>' } ], loop: false, initialScale: 'scale',controlsOverVideo: 'locked', autoBuffering: true, useNativeFullScreen: true, autoPlay: false }");
                          fo.write("fp1");
                      	</script>
                    ...
                    <video width="<?=$video_width;?>" height="<?=$video_width*.567;?>" controls autoplay style="position:absolute;top:40px;right:10px;border:4px solid white;">
										  ...
										  if($selected_playlist == $playlist){?>
										  	<source src="http://claytonfelt.net/mypage1/videos/<?=$video;?>" type="video/mp4">
										  <?php 
											} ?>
